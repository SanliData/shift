...
## Araç QR Yönetimi ve Yazdırma

1. `/admin-time/vehicles` sayfasına git.
2. "Yeni Araç / İş Makinesi Ekle" formunda `name`, `type`, `qr_code_slug` gir:
   - `qr_code_slug` boş bırakılırsa otomatik `slug` üretilir (`Excavator-02` -> `excavator-02`).
3. Aktif araçlar "Aktif Araçlar (QR Üretimi)" bölümünde QR ile listelenir.
4. "QR indir (print)" butonu ile siyah-beyaz, büyük ve yazdırılabilir PNG indir.
5. Çıktıyı A4 üzerine yazdırıp laminasyon için araç üzerine araç adı/ID görünür şekilde kullan.
6. Araç silmek yerine "Pasifleştir" kullanılır (`active=false`); eski mesai kayıtları korunur.
